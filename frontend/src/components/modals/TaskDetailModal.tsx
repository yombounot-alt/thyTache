    ...
    updateTask.mutate(
      { id: task.id, patch: { status }, actorId: user.id },
      {
        onSuccess: () => {
          ...
          if (status === "done" && task.creatorId !== user.id) {
            notificationService.create(task.creatorId, {
              type: "task_completed",
              title: "Tâche terminée",
              message: `${user.firstName} a marqué « ${task.title} » comme terminée.`,
            })
          }
        },
        onError: (error) => {
      ...
      { taskId: task.id, authorId: user.id, content: comment.trim() },
      {
        onSuccess: () => {
          ...
          if (task.assigneeId && task.assigneeId !== user.id) {
            notificationService.create(task.assigneeId, {
              type: "task_comment",
              title: "Nouveau commentaire",
              message: `${user.firstName} a commenté « ${task.title} ».`,
            })
          }
        },
        onError: (error) => {
